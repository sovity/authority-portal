...
class CatalogApiTest {
    ...
    fun `test two dataspaces and filter for one`() {
        ...
        assertThat(result.dataOffers.first().connectorId).isEqualTo("${dummyDevMdsId(0)}.${dummyDevConnectorId(1)}")
    }
    ...
    fun `test available filters - no filter`() {
        ...
        assertThat(connectorId.values).allSatisfy { it.id in setOf("${dummyDevMdsId(0)}.${dummyDevConnectorId(0)}") }
    ...
    fun `test sorting by popularity`() {
        ...
        repeat(3) { dataOfferDetails("${dummyDevMdsId(0)}.${dummyDevConnectorId(0)}", dummyDevAssetId(0)) }
