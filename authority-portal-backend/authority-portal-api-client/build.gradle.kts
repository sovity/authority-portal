...
dependencies {
    ...
    openapiYaml(project(":authority-portal-api")) {
        isTransitive = false
    }
...
val openapiFile = "authority-portal.yaml"
task<Copy>("extractOpenapiYaml") {
    ...
    doNotTrackState("Gradle can't track the file in the jar at the given time.")
}
...
tasks.getByName<org.openapitools.generator.gradle.plugin.tasks.GenerateTask>("openApiGenerate") {
    ...
    configOptions.set(mutableMapOf(
            "invokerPackage" to "de.sovity.authorityportal.client.gen",
            "apiPackage" to "de.sovity.authorityportal.client.gen.api",
            "modelPackage" to "de.sovity.authorityportal.client.gen.model",
...
tasks.withType<org.gradle.jvm.tasks.Jar> {
    // This is explicitly required due to the sourcesJar
    dependsOn("postprocessGeneratedClient")
}
